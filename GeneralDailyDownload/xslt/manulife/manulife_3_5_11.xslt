...
>
  ...
  <xsl:variable name="special" select="'&#34;'"/>
    ...
    <xsl:for-each select="//table[@id='viewns_Z7_01A41A42IGEOE0A0S3EDN81020_:mainContent:alphaBond']/descendant::tr[position()>1]">
      <xsl:text>&#xa;</xsl:text>
    ...
    <xsl:for-each select="//table[@id='viewns_Z7_01A41A42IGEOE0A0S3EDN81020_:mainContent:alphaOther']/descendant::tr[position()>1]">
      <xsl:text>&#xa;</xsl:text>
      <xsl:variable name="Name" select="concat($special,./td[@class = 'second'],$special)"/>
      <xsl:variable name="Code" select="concat($special,./td[@class = 'first'],$special)"/>
      <xsl:variable name="Currency" select="'USD'"/>
    ...
    <xsl:for-each select="//table[@id='viewns_Z7_01A41A42IGEOE0A0S3EDN81020_:mainContent:alphaInternal']/descendant::tr[position()>1]">
      <xsl:text>&#xa;</xsl:text>
      <xsl:variable name="Name" select="concat($special,./td[@class = 'second'],$special)"/>
      <xsl:variable name="Code" select="concat($special,./td[@class = 'first'],$special)"/>
      <xsl:variable name="Currency" select="'USD'"/>
