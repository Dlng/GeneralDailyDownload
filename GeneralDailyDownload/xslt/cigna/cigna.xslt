...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="//table[@class='infoTable formsList']/tbody/descendant::tr[position()>2 and position() != last()]">
      ...
      <xsl:variable name="Currency">
        <xsl:value-of select="concat($special,customfuns:replace(substring-before(substring-after(//table[@class='infoTable formsList']/tbody/descendant::tr[position()=2]/th[position()=3],'('),')'),'$',''),$special)"/>
      </xsl:variable>
