...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="//table[@class='cms-table']/tbody/descendant::tr[position()>1]">
      ...
      <xsl:variable name="PriceDate">
        <xsl:value-of select="concat($special,substring-after(//span[@class = 'h1 z-label'],'-'),$special)"/>
      </xsl:variable>

      <xsl:variable name="Currency">
        <xsl:value-of select="concat($special,substring-before(substring-after(//table[@class='cms-table']/tbody/descendant::tr[position()=1]/th[position()=2],'('),')'),$special)"/>
      </xsl:variable>
      ...
      <xsl:variable name="NAV" select="concat($special,normalize-space(./td[position()=3]),$special)"/>
