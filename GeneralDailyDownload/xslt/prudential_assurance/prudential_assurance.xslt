...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="//table[@class='daily']/tbody/descendant::tr[@class = 'dataRow1' or class = 'dataRow2']">
      ...
      <xsl:variable name="Currency">
        <xsl:value-of select="concat($special,substring-before(substring-after(//table[@class='daily']/tbody/descendant::tr[position()=1],'('),')'),$special)"/>
      </xsl:variable>
      ...
      <xsl:variable name="NAV" select="concat($special,./td[position()=2],$special)"/>
