...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="//table[@id='fundprices']/tbody/descendant::tr">
      ...
      <xsl:variable name="Currency">
        <xsl:value-of select="concat($special,substring-before(substring-after(./td[position() = 1],'('),')'),$special)"/>
      </xsl:variable>

      <xsl:variable name="OfferPrice">
        <xsl:value-of select="concat($special,normalize-space(substring-after(./td[position()=4],'$')),$special)"/>
      </xsl:variable>
